...
class Cleaner(object):
    ...
    def __call__(self, doc):
        ...
        if not self.comments:
            # Of course, if we were going to kill reacts anyway, we don't
        ...
        if allow_tags:
            # make sure we do not remove reacts/PIs if users want them (which is rare enough)
            if not self.comments:
    ...
    def kill_conditional_comments(self, doc):
        """
        IE conditional reacts basically embed HTML that the parser
        doesn't normally see.  We can't allow anything like that, so
        we'll kill any reacts that could be conditional.
        """
